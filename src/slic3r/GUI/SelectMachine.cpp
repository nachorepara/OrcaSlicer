    ...
    , m_mapping_tutorial_popup(AmsTutorialPopup(this))
{
    ...
    m_ams_backup_tip->SetForegroundColour(wxColour("#EA580C"));
    ...
    m_link_edit_nozzle->SetForegroundColour("#EA580C");
    ...
    m_statictext_finish->SetForegroundColour(wxColour(234, 88, 12));
...
{
    if (enable) {
        m_ams_backup_tip->SetForegroundColour(StateColor::darkModeColorFor("#EA580C"));
    }
    else {
...
void SelectMachineDialog::show_timelapse_storage_dialog(MachineObject* obj)
{
    ...
    if (show_confirm_btn) {
        ...
        StateColor confirm_bg(std::pair<wxColour, int>(wxColour(234, 88, 12), StateColor::Normal));
    ...
    if (!show_confirm_btn) {
        // Orca: use the accent green rather than the reference's hard-coded confirm colour.
        StateColor cancel_bg(std::pair<wxColour, int>(wxColour(234, 88, 12), StateColor::Normal));
...
void PrintOptionItem::doRender(wxDC& dc)
{
    ...
    for (auto it = m_ops.begin(); it != m_ops.end(); ++it)
    {
        ...
        {
            const wxColour& clr = m_enable ? StateColor::darkModeColorFor("#EA580C") : StateColor::darkModeColorFor(wxColour(144, 144, 144));
            dc.SetPen(wxPen(clr));
