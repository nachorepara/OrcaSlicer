    ...
    m_task_type(type)
{
    ...
    auto m_btn_bg_enable = StateColor(
        std::pair<wxColour, int>(wxColour(194, 65, 12), StateColor::Pressed),
        std::pair<wxColour, int>(wxColour(249, 115, 22), StateColor::Hovered),
        std::pair<wxColour, int>(wxColour(234, 88, 12), StateColor::Normal)
    );
        ...
        if (get_obj()) {
            if (state_cloud_task == 0 && m_job_id == get_obj()->profile_id_) {
                ...
                if (state_device == 0) {
                    ...
                    dc.SetTextForeground(wxColour(234, 88, 12));
                    DrawTextWithEllipsis(dc, get_state_device(), FromDIP(DEVICE_LEFT_PRO_INFO), left);
                }
                else if (state_device == 2)
                {
                    ...
                    if (obj_->get_curr_stage() == _L("Printing") && obj_->subtask_) {
                        ...
                        dc.SetPen(wxPen(wxColour(234, 88, 12)));
                        dc.SetBrush(wxBrush(wxColour(234, 88, 12)));
